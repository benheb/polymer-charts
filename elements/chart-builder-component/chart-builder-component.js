(function () {
  ...
  Polymer({
    ...
    change: function(e) {
      this.url = this.$.urlinput.value + "query?where=1%3D1&outFields=*&returnGeometry=false&f=json";
      this.$.getAttributes.go();
    },

    changeColor: function(e) {
      var color = e.detail.msg;
      this.options.pattern = color;
      if ( this.chart ) {
        var selected = this.selectedAttribute;
        var c = {};
        c[selected] = color;
        this.chart.data.colors(c);
      }
    },

    onAttributesFetched: function(res) {
      var self = this;
      this.fields = res.detail.response.fields;
      this.data = res.detail.response;

      $(self.$.attributeList).empty();
      _.each(this.fields, function(field) {
        var html = '<div class="attr-name" label="'+field.name+'">'+field.name+'</div>';
        $(self.$.attributeList).append(html);
      });
    },
    ...
    buildOptions: function() {
      ...
      _.each(res.fields, function(f, i) {
        if (f.name === self.selectedAttribute ) {
          field = f;
        }
      });
      ...
      self.options.xLabel = self.selectedAttribute;
