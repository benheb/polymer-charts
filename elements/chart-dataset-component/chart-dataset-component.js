(function () {
  ...
  Polymer({
    ...
    responseChanged: function(oldValue) {
      ...
      if ( res !== null && res.features ) {
        var options = this.buildOptions(res);
        _.each(options, function(opt) {
          if ( opt.xLabel === "ELEV" ) {
            self.initChart(opt);
            self.count = res.features.length;
            self.selected = opt.xLabel;
            self.bins = opt.values[0].length;
          }
        });
      }
    },
    ...
    buildOptions: function(res) {
      ...
      _.each(res.fields, function(field, i) {
        var  regions = null,
          values = [],
          xAxis = [],
          hist,
        ...
        //add chart object to data array
        data.push({
          values: [values],
          xAxis: xAxis,
          regions: [],
    ...
    *
    */
    _abbrIds: function(ids) {
      var s = _.sortBy(ids, function(num) {
        return num.length;
      });
      s.reverse();
      var small = s.slice(0, 10);
      var other = s.slice(10, s.length);
      var sum = _.union(other);
      sum = _.reduceRight(sum, function(a, b) { return a.concat(b); }, []);
      small.push(sum);
    ...
    *
    */
    _abbrStringValues: function(values) {
      var s = _.sortBy(values, function(num) {
        return num;
      });
      s.reverse(); 
      var small = s.slice(0, 10);
      var other = s.slice(10, s.length - 1);
      var sum = _.reduce(other, function(memo, num){ return memo + num; }, 0);
      small.push(sum);
    ...
    _abbrStringxAxies: function(values) {
      var s = _.sortBy(values, function(num) {
        return num;
      });
      s.reverse(); 
    ...
    /*
    * Regions help break up chart background in UI 
    * Calculate best number of regions and where they break based on bins
    * @param {number} len    length of bin array
    */
    _getRegions: function(len) {

      var step = len / 9;
      var regions = [];
      regions.push({axis: "x", end: step, "class": "x-region"});

      var i = 5, itr = step;
      while (i > 0) {
        regions.push({axis: "x", start: itr + step, end: itr + (step * 2), "class": "x-region"});
        itr += (step * 2);
        i--;
      }
      return regions;
